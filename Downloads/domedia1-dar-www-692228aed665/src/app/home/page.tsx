...
export default async function Home() {
	...
	const trendSectioneContent = await getPageContent({
		query: {
			section: "trend",
		},
	});

	const newsSection = pageContent?.find((s) => s.section === "news");
	const podcastsSection = pageContent?.find((s) => s.section === "podcasts");
	const soundAndImageSection = pageContent?.find(
		(s) => s.section === "sound-and-image"
	);
	console.log({ economicPageContent: economicSectionContent });
	return (
		<Container className="my-6">
			<Titles.Title text="أهم الأخــبــار" className="text-center my-6" />
			...
			<section className="flex gap-8 my-8">
				...
				<div className="flex-2/3">
							...
							>
								<div>
									<Image
										...
										src={newsSection?.contents?.[1]?.s3_url}
										width={350}
										height={200}
										alt="Sunset in the mountains"
									/>
					...
					<div className="grid grid-cols-2 gap-x-12 gap-y-4 mt-8">
						{newsSection?.contents?.slice(2, 4).map((item) => (
							<Link
								href={"/"}
			...
			<section>
				...
				<div className="my-8">
					<Buttons.ReadMore text="اعرض المزيد" href="/" />
				</div>
			</section>

			{/* Most Read */}
			<section>
				<Titles.Title text="الأكثر قراءة" className="my-10 text-4xl" />
							...
							{[0, 1, 2].map((item) => (
								...
								>
									...
									<div className="border-b border-[#D9D9D9] pb-4 flex gap-4 items-center">
										...
										>
											جيش الاحتلال الإسرائيلي يقر بمقتل 7
											من جنوده في معارك جنوبي غزة
										</Link>
									</div>
								</div>
							))}
